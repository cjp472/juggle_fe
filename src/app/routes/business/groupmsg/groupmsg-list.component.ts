...
import { GroupmsgListEditComponent } from './edit/edit.component';

@Component({
  selector: 'app-groupmsg-list',
  templateUrl: './groupmsg-list.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class GroupmsgListComponent implements OnInit {
  ...
  setupFilters() {
    ...
    this.filters.push(new Field('input', '内容', 'content'));
    this.filters.push(
      new Field('select', '状态', 'status', [
        { label: '待推送', value: 'WAIT' },
        { label: '已推送', value: 'COMPLETE' },
      ]),
    );
  }
  ...
  setupTable() {
    this.columns = [
      ...
      { title: '群发时间', index: 'sendTime', type: 'date' },
      {
        title: '状态',
        index: 'status',
        type: 'badge',
        badge: {
          WAIT: { text: '待群发', color: 'processing' },
          COMPLETE: { text: '已群发', color: 'success' },
        },
      },
  ...
  openEdit(record: any = {}) {
    this.modal.create(GroupmsgListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      if (record.id) {
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/business/groupSms/' + item.id + '/remove',
        {},
        {
          success: res => {
            this.reset();
