...
import { PushListEditComponent } from './edit/edit.component';
...
export class PushListComponent implements OnInit {
  ...
  setupTable() {
    this.columns = [
      ...
      { title: '推送时间', index: 'pushTime', type: 'date' },
      {
        title: '状态',
        index: 'status',
        type: 'badge',
        badge: {
          WAIT: { text: '待推送', color: 'processing' },
          COMPLETE: { text: '已推送', color: 'success' },
        },
      },
  ...
  openEdit(record: any = {}) {
    this.modal.create(PushListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      if (record.id) {
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/business/push/' + item.id + '/remove',
        {},
        {
          success: res => {
            this.reset();
