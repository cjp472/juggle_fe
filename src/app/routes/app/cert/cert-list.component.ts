...
export class CertListComponent implements OnInit {
  ...
  setupFilters() {
    this.filters.push(
      ...
      new Field('dateRange', '申请时间', 'applyDateRange'),
    );
  }
  ...
  setupTable() {
    this.columns = [
      ...
      { title: '姓名', index: 'extra.realName' },
      { title: '手机号', index: 'member.mobile' },
      { title: '身份证', index: 'extra.idCard' },
  ...
  openView(record: any) {
    this.router.navigateByUrl(`/app/cert/${record.id}`);
  }
  ...
  filterHandel(filters: Array<Field>) {
    const handels: Array<any> = [{ name: 'processInstanceType', value: 'CERT' }];
