...
import { ShopSkutypeListEditComponent } from './edit/edit.component';

@Component({
  selector: 'app-skutype-list',
  templateUrl: './skutype-list.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ShopSkutypeListComponent implements OnInit {
  ...
  getData(pi = 1) {
    ...
    this.httpService.post(
      '/v1/shop/shopProductSkuType/search',
  ...
  openEdit(record: any = {}) {
    this.modal.create(ShopSkutypeListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      if (record.id) {
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/shop/shopProductSkuType/' + item.id + '/remove',
        {},
        {
          success: res => {
            this.reset();
