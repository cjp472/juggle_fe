...
import { TaobaoTypeListEditComponent } from './edit/edit.component';
...
export class TaobaoTypeListComponent implements OnInit {
  constructor(
  ...
  getData() {
    ...
    this.httpService.post(
      '/v1/taobao/taobaoType/searchTree',
  ...
  openEdit(record: any = {}) {
    this.modal.create(TaobaoTypeListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      this.reset();
      this.cdr.detectChanges();
    });
  }
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/taobao/taobaoType/' + item.id + '/remove',
        {},
        {
          success: res => {
  ...
  changeEnabled(item: any) {
    ...
    let url = '/v1/taobao/taobaoType/' + item.id + '/disable';
    if (!item.enabled) {
      content = '确定要启用该商品分类吗?';
      url = '/v1/taobao/taobaoType/' + item.id + '/enable';
    }
