...
import { UnitListEditComponent } from './edit/edit.component';
...
export class UnitListComponent implements OnInit {
  ...
  setupFilters() {
    this.filters.push(new Field('input', '组织名', 'name'));
    this.filters.push(
  ...
  getData() {
    ...
    this.httpService.post(
      '/v1/system/organization/searchTree',
  ...
  openEdit(record: any = {}) {
    this.modal.create(UnitListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      this.reset();
      this.cdr.detectChanges();
    });
  }
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/system/organization/' + item.id + '/remove',
        {},
        {
          success: res => {
  ...
  changeEnabled(item: any) {
    let content = '确定要禁用该组织吗?';
    let url = '/v1/system/organization/' + item.id + '/disable';
    if (!item.enabled) {
      content = '确定要启用该组织吗?';
      url = '/v1/system/organization/' + item.id + '/enable';
    }
