...
import { TaskScheduleListEditComponent } from './edit/edit.component';
...
export class TaskScheduleListComponent implements OnInit {
  ...
  openEdit(record: any = {}) {
    this.modal.create(TaskScheduleListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      if (record.id) {
  ...
  filterHandel(filters: Array<Field>) {
    const handels: Array<any> = [{ name: 'type', value: 'TIME' }];
