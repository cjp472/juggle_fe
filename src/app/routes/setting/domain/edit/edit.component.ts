...
@Component({
  selector: 'app-domain-list-edit',
  templateUrl: './edit.component.html',
})
export class DomainListEditComponent {
  record: any = {};
  schema: SFSchema = {
    properties: {
      chains: {
        ...
        ui: {
          ...
          asyncData: (node, index) => {
            return new Promise(resolve => {
              setTimeout(() => {
                this.httpService.getRepositoryOfTreeOptionData('/v1/setting/domain/searchTreeSelect').subscribe(res => {
  ...
  save(value: any) {
    let url = '/v1/setting/domain/create';
    if (this.record.id) {
      url = '/v1/setting/domain/' + this.record.id + '/update';
    }
