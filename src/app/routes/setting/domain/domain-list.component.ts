...
import { DomainListEditComponent } from './edit/edit.component';
...
export class DomainListComponent implements OnInit {
  ...
  openEdit(record: any = {}) {
    this.modal.create(DomainListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      this.reset();
      this.cdr.detectChanges();
    });
  }
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/setting/domain/' + item.id + '/remove',
        {},
        {
          success: res => {
  ...
  changeEnabled(item: any) {
    ...
    let url = '/v1/setting/domain/' + item.id + '/disable';
    if (!item.enabled) {
      content = '确定要启用该商品分类吗?';
      url = '/v1/setting/domain/' + item.id + '/enable';
    }
