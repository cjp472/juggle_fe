...
export class GuideListComponent implements OnInit {
  ...
  filterHandel(filters: Array<Field>) {
    const handels: Array<any> = [{ name: 'type', value: 'GUIDE' }];
    filters.forEach(filter => {
      if (filter.value !== undefined && filter.value !== null) {
        if (filter.type === 'input') {
          ...
          if (filter.value) {
            handels.push({ name: filter.name, op: 'is_null', value: true });
          } else {
            handels.push({ name: filter.name, op: 'is_null', value: false });
          }
        } else {
