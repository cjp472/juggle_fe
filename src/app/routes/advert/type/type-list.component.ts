...
import { AdvertTypeListEditComponent } from './edit/edit.component';
...
export class AdvertTypeListComponent implements OnInit {
  ...
  openEdit(record: any = {}) {
    this.modal.create(AdvertTypeListEditComponent, { record }, { size: 'md' }).subscribe(res => {
      if (record.id) {
  ...
  remove(item: any) {
    this.modalService.showDeleteConfirm('确定要删除吗?', () => {
      this.httpService.post(
        '/v1/advert/advertType/' + item.id + '/remove',
        {},
        {
          success: res => {
            this.reset();
